...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.extra_layer = nn.Sequential(layer_init(nn.Linear(num_linear_units, num_linear_units), std=0.1), nn.ReLU())
        ...
        self.critic_ext = layer_init(nn.Linear(num_linear_units, 1), std=0.01)
        self.critic_int = layer_init(nn.Linear(num_linear_units, 1), std=0.01)
...
if __name__ == "__main__":
    ...
    run_name = f"ppo_rnd_{env_name}_s{args.seed}"
    ...
    num_channels = envs.single_observation_space["obs"].shape[-1]
    ...
    obs_rms = RunningMeanStd(shape=(1, num_channels, 10, 10))
    ...
    next_ob = []
    print("Running for {} steps...".format(args.num_steps * args.num_iterations_obs_norm_init))
    for step in range(args.num_steps * args.num_iterations_obs_norm_init):
        acs = np.random.randint(0, envs.single_action_space.n, size=(args.num_envs,))
        s, r, d, trunc, _ = envs.step(acs)
        next_ob += s["obs"].reshape([-1, num_channels, 10, 10]).tolist()
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            rnd_next_obs = (
                (
                    (next_obs.reshape(args.num_envs, num_channels, 10, 10) - torch.from_numpy(obs_rms.mean).to(device))
            ...
            for info in infos["final_info"]:
                ...
                avg_returns.append(info['episode']['r'][0] / info["deaths"])
                epi_ret = np.average(avg_returns)
                print(
                    f"global_step={global_step}, episodic_return={info['episode']['r'][0] / info['deaths']}, curiosity_reward={np.mean(curiosity_rewards[step].cpu().numpy())}"
                )
                ...
                writer.add_scalar(
                    ...
                    np.mean(curiosity_rewards[step].cpu().numpy()),
                    global_step,
                )
                ...
                logger_.logs_a([
                    global_step,
                    info["episode"]["r"][0] / info["deaths"],
                    info["episode"]["l"][0] / info["deaths"],
        ...
        obs_rms.update(b_obs.reshape(-1, num_channels, 10, 10).cpu().numpy())
        ...
        rnd_next_obs = (
            (
                (b_obs.reshape(-1, num_channels, 10, 10) - torch.from_numpy(obs_rms.mean).to(device))
                / torch.sqrt(torch.from_numpy(obs_rms.var).to(device))
            ).clip(-5, 5)
        ).float()
